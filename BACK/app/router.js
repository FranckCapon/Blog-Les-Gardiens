...
// implémentation de joi, avec un validator déja écrit dans le dossier "services" et un example de shéma dans le dossier "schema".
const {
  validateQuery,
  validateBody,
  validateParams
...
// Pour la mise en place d'une route sécurisé via la vérification d'un token : express-jwt et express-jwt-blacklist pour blacklister des tokens.
var guard = require('express-jwt-permissions')();
const expressjwt = require('express-jwt');
const blacklist = require('express-jwt-blacklist');

//la config de express-jwt-blacklist :
// le jti est le JWT ID, l'identifiant unique du token, construit dans la méthode handleLoginForm du userController via le pseudo de l'utilisateur et un nombre random a 6 chiffres.
blacklist.configure({
  tokenId: 'jti'
});
...
//Config de test pout POSTMAN : Header => 2 keys-value // Content-Type = application/x-www-form-urlencoded et Authorization = Bearer <TOKEN>
//Dans le body, on doit cocher : x-www-form-urlencoded
//Pour le Token, il faut faire une requete POST sur /connexion avec un user en BDD. La requete renvoie un Token. on le copie et on le colle a la place de <TOKEN> dans la colonne value. Laisser un espace entre Bearer (nom issue d'une norme W3C) et le token.
...
//router.get('/inscription', userController.signupForm);
/**
 * Une inscription
...
router.post('/user/reset_pwd', userController.reset_pwd);
...
console.log('je suis dans le router avant ma route evenements')
/**
